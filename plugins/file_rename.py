...
async def auto_rename_files(client, message):
    ...
    try:
        ...
        if _bool_metadata:
            ...
            if metadata:
                cmd = f'ffmpeg -i "{renamed_file_path}"  -map 0 -c:s copy -c:a copy -c:v copy -metadata title="GenAnimeOfc [t.me/GenAnimeOfc]" -metadata author="DARKXSIDE78" -metadata encoded_by="GenAnimeOfc" -metadata:s:s title="{metadata}" -metadata:s:a title="{metadata}" -metadata:s:v title="{metadata}"  "{metadata_file_path}"'
                try:
